...
class VariableEditor extends React.PureComponent {
    ...
    prepopInput = variable => {
        if (this.props.onEdit !== false) {
            ...
            this.setState({
                ...
                editType: toType(variable.value),
                parsedInput: {
                    type: detected.type,
    ...
    submitEdit = (submit_detected) => {
        const { variable, namespace, rjvId, fixedType } = this.props;
        const { editValue, editType, parsedInput } = this.state;
        let new_value = editValue;
        if (fixedType && editType) {
            if (editType === 'null' || editType === 'undefined' || editType === 'string') {
                new_value = editValue;
            } else if (editType === parsedInput.type) {
                new_value = parsedInput.value;
            } else {
                dispatcher.dispatch({
                    name: 'VARIABLE_TYPE_INVALID',
                    rjvId: rjvId,
                });
                return;
            }
        } else if (submit_detected && parsedInput.type) {
            new_value = parsedInput.value;
        }
        this.setState({
            editMode: false
